...
def make_labels_for_ncbi_from_hmmer():
    """
    Reads hmmer output (EMBL UniProt labels) and makes list of labels for NCBI
    :param file: hmmer output file 'download.fa'
    :return: list of labels for NCBI
    """
    with open("download.fa", "r") as download_fa, open(
        "proteins.fa", "w"
    ) as output_prots:
...
def download_ncbi_proteins(ncbi_labels):
    """
    Download .gbk files about proteins from NCBI
    :param ncbi_labels: list of labels for NCBI
    :return: list of processed labels
    :return file: gb_prot.gbk file with proteins
    :return file: proteins_2.fa file with proteins
    """
    with open("gb_prot.gbk", "w") as pos_file, open(
        "proteins.fa", "r"
    ) as input_prot, open("proteins_2.fa", "w") as output_prot:
        ...
        for idx, label in enumerate(ncbi_labels):
            ...
            logging.log(final_position.text)
        # Write gbk in file
        pos_file.write("\n".join(gbk_protein))
...
def find_places():
    """
    Find places in genome of proteins
    :param file: gb_prot.gbk file with proteins
...
def parce_dna(places, protein_labels) -> dict:
    """
    Parce DNA sequence from NCBI
    :param places: list of places in genome
    :param protein_labels: list of labels for proteins
    :return dictionary: dict of genomes
    """
    ...
    for idx, place in enumerate(places):
        ...
        logging.log(protein_labels[idx], id)
    ...
    with open("proteins_2.fa", "r") as input_prot, open(
        "proteins_3.fa", "w"
    ) as output_prot:
        ### Process proteins file
        input_lines = input_prot.read().splitlines()
...
def install_files(sigma):
    ...
    :return files: hmm files
    """
    ### Download the necessary files form github repository
    directory_path = pathlib.Path(__file__).parent.resolve() + "/HMMs"

    try:
        with open("HMMs/Sigma70_r2.hmm", "r") as f:
            pass
    except IOError:
    ...
    try:
        with open("HMMs/Sigma70_r4.hmm", "r") as f:
            pass
    except IOError:
...
def find_regions(dictionary):
    ...
    :param files: HMM files
    :param dictionary: dictionary of labels
    :return: files with r2 and r4 domens in protein sequence and dna promotor with -10 & -35 motifs
    """
    ...
    def parce_motifs():
        ...
        """
        # Proteins r2 domen
        process_prot_r2 = Popen(
            [
                "hmmsearch",
                "-A",
                "temp_protein_r2.txt",
                "HMMs/Sigma70_r2.hmm",
                "temp_current_prot.fa",
            ],
            stdout=PIPE,
            stderr=PIPE,
        )
    ...
    def check_values(stdout) -> bool:
        """
        The function checks if hmmer found anything
        """
    ...
    def byte_str_operating(protein_id, stdout_r2, stdout_r4, genome):
        ...
        :param stdout_r2: console output for r2 domen
        :param stdout_r4: console output for r4 domen
        :param genome: genome sequence
        :return: r2 and r4 domens in protein sequence and dna promotor with -10 & -35 motifs
        """
    ...
    def get_idseq(fasta):
        """
        The function extracts id and sequence from fasta file
    ...
    with open("dna_seq.fa", "r") as dna, open("proteins_3.fa", "r") as prot:
        ...
        with open("prot_r2.fa", "w") as prot_r2, open(
            "prot_r4.fa", "w"
        ) as prot_r4, open("promoters.fa", "w") as dna_r, open(
            "temp_trash.txt", "w"
        ) as trash:
            ...
            for genome in genomes_list:
                ...
                logging.log(protein_id, gene_id)
                ...
                protein_r2, protein_r4, dna_r_list = byte_str_operating(
                    protein_id, stdout_r2, stdout_r4, gene_seq
                )
...
def protdnakorr_adaptation():
    """
    The function makes adaptation of the data for PDK
    : param files: prot_r2_short.fa, prot_r4.fa, promoters.fa
    :return files: final_r2_short_-2.txt, final_r4.txt
    """
    ...
    with open("prot_r2_short.fa", "r") as r2, open("prot_r4.fa", "r") as r4, open(
        "promoters.fa", "r"
    ) as r, open("final_r2_short_-2.txt", "w") as f2, open("final_r4.txt", "w") as f4:
