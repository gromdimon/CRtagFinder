...
def install_files():
    ...
    try:
        with open("HMMs/AlgU.hmm", "r") as f:
            pass
    except IOError:
...
def find_regions():
    ...
    def parce_motifs():
        ...
        process_dna_r2 = Popen(
            ["nhmmer", "-o", "temp_dna_r.txt", "AlgU.hmm", "temp_current_dna.fa"],
            stdout=PIPE,
            stderr=PIPE,
        )
    ...
    with open("dna_seq.fa", "r") as dna, open("download.fa", "r") as prot:
        ...
        with open("proteins_r2_for_align.fa", "w") as prot_r2, open(
            "proteins_r4_for_align.fa", "w"
        ) as prot_r4, open("dna_for_align.fa", "w") as dna_r, open(
            "dna_for_lengthening.fa", "w"
        ) as leng_dna:
            for genome, protein in zip(genomes_list, proteins_list):
                ...
                protein_r2, protein_r4, dna_r_list = byte_str_operating(
                    protein_id, stdout_r2, stdout_r4, gene_id
                )
...
def aligning_dna():
    process = Popen(
        [
            "clustalw",
            "-INFILE=dna_for_align.fa",
            "-OUTPUT=FASTA",
            "-OUTFILE=aligned_dna.fa",
        ],
        stdout=PIPE,
        stderr=PIPE,
    )
...
def lengthening():
    ...
    with open("dna_for_lengthening.fa", "r") as dna_find, open(
        "aligned_dna.fa", "r"
    ) as aligned_motifs, open("lengthed_dna.fa", "w") as output:
...
def protdnakorr_adaptation():
    ...
    with open("prot_r2_modified.fa", "r") as r2, open(
        "prot_r4_modified.fa", "r"
    ) as r4, open("lengthed_dna_hrpl.fa", "r") as r, open(
        "final_r2.txt", "w"
    ) as f2, open(
        "final_r4.txt", "w"
    ) as f4:  #
